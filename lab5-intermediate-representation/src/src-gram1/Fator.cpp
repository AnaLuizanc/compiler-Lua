...
Exp* Fator::gerar_IR() {
    if (valor == "PAREN") 
        ...
        return new ExpConst(1); 
    else if (valor == "false")
        return new ExpConst(0); 
    else if (valor.length() > 0 && isdigit(valor[0])) 
        return new ExpConst(stoi(valor));
    else if (valor == "-") {
        Exp* zero = new ExpConst(0);
        Exp* exp_interna = interno->gerar_IR();
        if (exp_interna != nullptr) 
            return new ExpBinop(new OperadorSub(), zero, exp_interna);
    }
    else {
        if (acesso_frame != nullptr) {
